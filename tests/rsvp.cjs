...
const guests=rosterContext.window.INVITADOS;assert.equal(guests.length,42);assert.equal(guests.reduce((s,g)=>s+g.cupos,0),88);
...
for(const id of ['ma-041','ma-042']){assert(server.saveResponse({action:'save',id,respuesta:'si'}).ok);assert.equal(rows.at(-1)[3],1);}
now='2026-10-02T05:00:00Z';assert.equal(server.saveResponse({action:'save',id:'ma-001',respuesta:'si'}).ok,false);
console.log('OK: 42 invitados/88 cupos, guardado, actualización, padrón fiable, lectura y fecha límite.');
...
async function panel(mode){
 ...
 assert.equal(get('#stat-pending').textContent,42);assert.equal(get('#stat-si').textContent,0);assert.equal(get('#rows').children.length,42);
 ...
 assert.equal(get('#stat-si').textContent,2);assert.equal(get('#stat-seats-si').textContent,6);assert.equal(get('#stat-no').textContent,1);assert.equal(get('#stat-pending').textContent,39);
